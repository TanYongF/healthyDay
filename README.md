...
| 登陆平台 |  1   | 每日首次 | 已完成 ✔️ |  |
| 填写血糖 | 1 | 一月三次 | 已完成 ✔️ |  |
| 填写胰岛素含量 | 2 | 一月一次 | 已完成 ✔️ |  |
| 完善用户信息 | 3 | 一年一次 | 已完成 ✔️ |  |
| 填写并发症信息 | 2 | 一月一次 | 已完成 ✔️ |  |
...
| 3 | `type` | 积分类型 0:成长积分 1：可兑换积分 | bit(1) | | NO | | |
| 4 | `points` | 积分值 | int | | NO | | |
| 5 | `max_frequency_per_day` | 积分每日限制，为0即不限制 | int | | NO | | |
| 6 | `max_frequency_per_month` | 积分每月限制，为0即不限制 | int | | NO | | |
| 7 | `max_frequency_per_year` | 积分每年限制，为0即不限制 | int | | NO | | |
| 8 | `effective_day` | 生效时长，单位：day | int | | NO | | |
